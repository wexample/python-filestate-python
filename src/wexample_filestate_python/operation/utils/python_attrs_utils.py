                ...
                if not has_kw_only:
                    ...
                    for arg in call.args:
                        if (isinstance(arg.keyword, cst.Name) and 
                            arg.keyword.value == "kw_only" and
                            isinstance(arg.value, cst.Name) and
...
def _is_attrs_decorator(decorator: cst.Decorator) -> bool:
    ...
    if isinstance(base_decorator, cst.Attribute):
        if (isinstance(base_decorator.value, cst.Name) and 
            base_decorator.value.value == "attrs" and
            isinstance(base_decorator.attr, cst.Name) and
            base_decorator.attr.value in ("define", "frozen")):
            return True
    
    # Check for @attr.s
    if isinstance(base_decorator, cst.Attribute):
        if (isinstance(base_decorator.value, cst.Name) and 
            base_decorator.value.value == "attr" and
            isinstance(base_decorator.attr, cst.Name) and
